// ═══════════════════════════════════════════
// DogNav 热榜聚合（Express 端；Worker 端为 cloudflare/src/hotlist.mjs，逻辑保持一致）
...
// ── B站热榜：优先 ranking/v2 全站榜；该接口对服务器/数据中心 IP 风控（-352）时
// 回退 popular 综合热门（风控尺度更松）。两个接口返回条目结构一致（title/bvid/stat.view）。
// Referer 必须是 www.bilibili.com，API 域名做 Referer 会被 -352 风控 ──
async function fetchBilibili() {
    ...
    for (const url of endpoints) {
        try {
            ...
            const list = data?.code === 0 ? data?.data?.list : null;
            ...
            lastErr = invalidPayload(`bilibili code ${data?.code}`);
        } catch (err) {
            lastErr = err; // 网络/HTTP 错误同样尝试下一个接口
        }
    }
    throw lastErr;
}
