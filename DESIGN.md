...
## Product goals
- Goals: 首屏完成「问候 → 搜索 → 常用 Dock → 轻量状态」的任务路径，随后在同一页面直接向下浏览全部分类站点；筛选栏（全部/精选/收藏/最近/热门/最新）轻量切换内容视角，分类跳转只滚动到对应分组；搜索、Dock、状态、筛选、分类跳转均单页可达
- Non-goals: 不做壁纸/登录/AI 搜索/小组件市场/监控图表；不引入前端框架、组件库、新字体或后端数据模型；不复制参考站的商标与个性功能
- Success signals: 桌面 1120px 内容宽、360–390px 无横向溢出；首访不点任何筛选/跳转即可向下看到每个分类的全部站点；热榜接口不回归；首屏外部请求仅 Google Fonts；常用 Dock 首访自动预填不留白
...
- Primary navigation: 无顶栏——右上角浮动日期时间；右下角文字按钮组（⊞ 全部站点/◷ 最近使用/⚙ 设置）+ 设置弹层（主题分段切换、清除本地数据、友链/关于/提交与自定义页链接）；无 Footer
...
- Tradeoffs: 单页连续展开换来「无需进入二级视图即可浏览全部」，页面变长后用吸顶筛选/跳转头部补偿定位；分类跳转行的激活态跟随滚动位置（scrollspy），而非用户点选；Dock 上限 12、展示前 10；问候语刻意小字低权重，避免滑向宣传式 Hero
...
## Responsive behavior
- Supported breakpoints/devices: 桌面 ≥1025px（内容 1120px、卡片 4 列、筛选/跳转头部 sticky 吸顶）；平板 ≤1024px（卡片 3 列）；手机 ≤768px（时钟隐藏、Dock 4 列网格、状态卡换行最近使用通栏其余两两均分、搜索占满减边距、kbd 提示隐藏、分类跳转行改 ☰ 抽屉且头部不吸顶、卡片 2 列）；≤480px（搜索 48px、Dock 图标 52px、sec 图标隐藏）
...
- Test/screenshot expectations: `node --test`（默认发现，兼容本机 Node 22）；`test/icon-repair-ui.test.js`、`test/home-ui.test.js` 等静态契约保持通过；截图验收 1440×900 / 390×844 × 浅色/深色
